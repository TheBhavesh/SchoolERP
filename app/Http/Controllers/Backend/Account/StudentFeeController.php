...
class StudentFeeController extends Controller
{
    public function StudentFeeView(){
		...
		$month = Month::with(['fee_category'])->get();
		// dd($month)->toArray();	
		// dd($month[1]->fee_category)->toArray();
		dd($month[1]['fee_category'][0]['id'])->toArray();


		$data = AssignStudent::with(['discount'])->where('year_id',3)->where('class_id',2)->get();
...
$accountstudentfees = AccountStudentFee::where('student_id',$std->student_id)->where('year_id',$std->year_id)->where('class_id',$std->class_id)
